...
public class EditDataActivity extends BaseActivity {
	...
	private void saveEntity() {
		...
		// Validation
		String validationMessage = new Validator(sqlHelper, mEntity).validate_dontcare_unique(mAttributes, mValues);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (resultCode == RESULT_OK && requestCode == SELECT_REQUEST_CODE) {
			...
			if (data.hasExtra(EditDataActivity.ENTITY_ID)) {

				Long selectedId = data.getExtras().getLong(EditDataActivity.ENTITY_ID);
				if (selectedId == null || selectedId.equals("0")) return;
				String refentityName = data.getExtras().getString(EditDataActivity.ENTITY_NAME);
				if (refentityName == null || refentityName.equals("")) return;

				String attributeName = data.getExtras().getString(EditDataActivity.ATTRIBUTE_NAME);
			...
			if (data.hasExtra(EditDataActivity.FILE_NAME)) {

				String filePath = data.getExtras().getString(EditDataActivity.FILE_NAME);
